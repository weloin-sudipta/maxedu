/**
 * usePdf — Reusable composable for PDF generation via MaxEdu PDF Service
 *
 * Usage:
...
export const usePdf = () => {
  ...
  const generateAdmitCard = (examType = "") =>
    _call("maxedu.api_folder.pdf.generate_admit_card_pdf", {
      exam_type: examType,
    });
  ...
  const generateFromTemplate = (templateName, data = {}, filename = "document.pdf", options = {}) =>
    _call("maxedu.api_folder.pdf.generate_pdf_from_template", {
  ...
  const generateFromHtml = (html, filename = "document.pdf", options = {}) =>
    _call("maxedu.api_folder.pdf.generate_pdf_from_html", {
      html,
      filename,
      options: JSON.stringify(options),
    });
