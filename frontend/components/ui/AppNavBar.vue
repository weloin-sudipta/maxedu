  ...
  <nav
    ...
    <div class="flex items-center flex-1 max-w-md">
      <div class="relative w-full group">
        ...
        <input
          ...
          class="block w-full pl-10 pr-12 py-2.5 bg-white border border-transparent rounded-xl text-sm text-gray-700 
        ...
        <div class="absolute inset-y-0 right-0 pr-3 hidden md:flex items-center pointer-events-none">
          <kbd class="px-1.5 py-0.5 text-[10px] font-semibold text-gray-400 bg-gray-100 border border-gray-200 rounded-md">
            ⌘ /
          </kbd>
        </div>
      </div>
    ...
    <div class="flex items-center gap-4">
      ...
      <div class="relative">
        ...
        <div v-if="isNotifOpen"
          ...
          <div class="p-4 border-b border-gray-50 flex justify-between items-center bg-white">
            ...
            <button class="text-[10px] font-bold text-indigo-600 uppercase hover:underline">Mark all as read</button>
          </div>
          <div class="max-h-[400px] overflow-y-auto custom-scrollbar">
            <div v-if="notifications.length > 0">
              <div v-for="notif in notifications" :key="notif.id"
                ...
                <div :class="['w-10 h-10 rounded-full flex items-center justify-center shrink-0', notif.bg]">
                  <i :class="['fa text-white', notif.icon]"></i>
                </div>
                <div class="flex-1">
                  <p class="text-xs font-bold text-gray-800 mb-1">{{ notif.title }}</p>
                  <p class="text-[11px] text-gray-500 line-clamp-2 leading-relaxed">{{ notif.message }}</p>
                  <p class="text-[9px] text-gray-400 mt-2 font-medium"><i class="fa fa-clock-o mr-1"></i>{{ notif.time }}</p>
                </div>
      ...
      <div class="flex items-center gap-3 pl-2">
        <button class="p-2 text-gray-500 hover:bg-indigo-50 hover:text-indigo-600 rounded-full transition duration-300">
      ...
      <div class="flex items-center gap-3 pl-2">
        <div class="flex-col items-end hidden sm:flex">
          <span class="text-sm font-semibold text-gray-700">{{ profileData.fullName || 'User' }}</span>
          <span class="text-xs text-gray-400 uppercase">{{ userRole || 'User' }}</span>
        </div>
        <div class="w-10 h-10 font-bold text-white bg-indigo-500 border-2 border-white rounded-full shadow-sm flex items-center justify-center">
