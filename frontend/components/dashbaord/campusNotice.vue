    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ notices.length }} updates</span>
        </div>
      </div>
      <button class="text-[10px] font-black text-indigo-600 uppercase tracking-widest bg-indigo-50 hover:bg-indigo-100 transition-colors px-3 py-1.5 rounded-full flex items-center gap-1.5">
    ...
    <div v-if="notices && notices.length > 0" class="flex flex-col">
      <div v-for="(notice, index) in notices" :key="notice.id"
        ...
        <div class="flex flex-col items-center flex-shrink-0 w-4 pt-1">
          ...
          <div v-if="index < notices.length - 1" class="w-px flex-1 min-h-[20px] mt-1 bg-gradient-to-b from-slate-200 to-transparent"></div>
        </div>
