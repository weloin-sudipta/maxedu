          ...
          <tbody class="divide-y divide-slate-50 dark:divide-slate-800/50 text-sm font-bold text-slate-700 dark:text-slate-300 transition-colors">
            <template v-for="fee in fees" :key="fee.id">
              <tr
                  ...
                  <div class="flex items-center gap-4">
                    <div :class="['w-8 h-8 rounded-xl flex items-center justify-center transition-colors',
                      expandedId === fee.id
                        ...
                        : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 group-hover:bg-indigo-100 dark:group-hover:bg-indigo-900/50']">
                ...
                <td class="p-6 text-slate-800 dark:text-slate-100 transition-colors">₹{{ fee.amount.toLocaleString() }}</td>
                <td class="p-6">
                  <span :class="[
                    fee.status === 'Paid'
                ...
                <td class="p-6 text-right">
                  <button v-if="fee.status === 'Unpaid'"
                    class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-transform active:scale-95 shadow-md shadow-indigo-200 dark:shadow-none">
                    Pay Now
                  </button>
                  <button v-else @click.stop="downloadFee(fee)"
                    class="text-slate-400 dark:text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 p-2 transition-colors">
                ...
                <td colspan="5" class="bg-slate-50/50 dark:bg-slate-800/30 p-0 transition-colors">
                  <div class="px-20 py-8 animate-in slide-in-from-top-2 duration-300">
                    <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm dark:shadow-none max-w-2xl transition-colors">
                      <h4 class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-6 transition-colors">
                        Detailed Fee Components
                      </h4>

                      <div class="space-y-4">
                        <div v-for="(item, index) in fee.breakout" :key="index"
                          class="flex justify-between items-center pb-3 border-b border-slate-50 dark:border-slate-800/50 last:border-0 transition-colors">
                          <div class="flex items-center gap-3">
                            <div class="w-1.5 h-1.5 rounded-full bg-indigo-400"></div>
                            <span class="text-slate-600 dark:text-slate-300 transition-colors">{{ item.label }}</span>
                          </div>
...
const transformFees = (data) => {
  ...
  return [{
    id: data.feesId || 'N/A',
    program_name: data.program || 'N/A',
    transactionId: data.feesId || 'N/A',
    amount: Number(data.total_fees) || 0,
    status: (Number(data.outstanding_amount) || 0) === 0 ? 'Paid' : 'Unpaid',
    date: data.due_date || 'N/A',
    words: data.grand_total_in_words || 'N/A',
    breakout: (data.components || []).map(c => ({
      label: c.fees_category || 'N/A',
