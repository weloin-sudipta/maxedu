        ...
        <div v-for="file in filteredFiles" :key="file.id" 
          ...
          <div class="flex-1 text-center md:text-left">
            <div class="flex flex-col md:flex-row md:items-center gap-2 mb-1">
              ...
              <span class="px-2 py-0.5 bg-slate-50 text-slate-400 text-[9px] font-black uppercase rounded border border-slate-100">{{ file.type }}</span>
            </div>
            <div class="flex flex-wrap justify-center md:justify-start gap-4">
              <span class="text-[10px] font-bold text-indigo-500 uppercase tracking-widest">{{ file.subject }}</span>
              <span class="text-[10px] font-bold text-slate-300 uppercase tracking-widest"><i class="fa fa-database mr-1"></i> {{ file.size }}</span>
              <span class="text-[10px] font-bold text-slate-300 uppercase tracking-widest"><i class="fa fa-calendar-o mr-1"></i> {{ file.date }}</span>
            </div>
          </div>

          <div class="flex items-center gap-3 shrink-0">
            <button class="w-12 h-12 flex items-center justify-center bg-slate-50 text-slate-400 rounded-xl hover:bg-slate-900 hover:text-white transition-all shadow-sm">
              <i class="fa fa-eye"></i>
            </button>
            <button class="px-6 py-3 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all">
              <i class="fa fa-download mr-2"></i> Download
            </button>
          </div>
        </div>

        <div v-if="filteredFiles.length === 0" class="bg-white rounded-[2.5rem] p-20 border border-dashed border-slate-200 text-center">
